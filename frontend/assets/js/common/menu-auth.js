...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  // Débogage
  console.log('Menu Auth - État utilisateur:', { isAuthenticated, isAdmin, user });
  ...
  if (isAuthenticated && user) {
    ...
    if (isAdmin && headerNav) {
      console.log('Utilisateur admin détecté, ajout du lien dashboard');
      
      // Vérifier si le lien d'administration existe déjà
      const adminLinkExists = Array.from(headerNav.querySelectorAll('a')).some(
        link => link.getAttribute('href').includes('admin/dashboard.html')
      );
      
      console.log('Lien admin existe déjà?', adminLinkExists);
      ...
      if (!adminLinkExists) {
        ...
        if (authContainer) {
          ...
          console.log('Lien admin ajouté avant le conteneur d\'authentification');
        } else {
          // Si le conteneur d'authentification n'existe pas, ajouter à la fin
          headerNav.appendChild(adminLink);
          console.log('Lien admin ajouté à la fin du header__nav');
        }
      }
    }
  } else {
